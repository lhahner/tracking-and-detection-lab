...
try:
    ...
    raise ImportError("SSNMMDetections3D requires MMDetection3D. Install the OpenMMLab stack first.") from exc
...
class SSNMMDetections3D(Detector):
    def __init__(self,
                 dataset,
                 classes,
                 settings,
    ...
    def __map_labels(self, labels_reference):
        ...
        if not torch.all(valid):
            raise ValueError("MMDetection3D SSN returned a class index outside"
