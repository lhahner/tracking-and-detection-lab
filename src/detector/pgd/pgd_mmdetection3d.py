...
try:
    ...
    raise ImportError("PDGMMDetections3D requires MMDetection3D. Install the OpenMMLab stack first.") from exc
...
class PGDMMDetections3D(Detector):
    def __init__(self,
                 dataset,
                 classes,
                 settings,
    ...
    def __map_labels(self, labels_reference):
        ...
        if not torch.all(valid):
            raise ValueError("MMDetection3D PGD returned a class index outside"
