...
MODEL_DEFAULTS = {
    "transfusion_openpcdet": {
        "output_name": "transfusion",
        "config": Path(ROOT_DIR)
        / "third_party/OpenPCDet/tools/cfgs/nuscenes_models/transfusion_lidar.yaml",
        "checkpoint": Path(ROOT_DIR)
        / "src/detector/transfusion/model/cbgs_transfusion_lidar.pth",
    },
    "voxelnext_openpcdet": {
        "output_name": "voxelnext",
        "config": Path(ROOT_DIR)
        / "third_party/OpenPCDet/tools/cfgs/nuscenes_models/cbgs_voxel0075_voxelnext.yaml",
        "checkpoint": Path(ROOT_DIR)
        / "src/detector/voxelnext/model/voxelnext_nuscenes_kernel1.pth",
    },
    "second_openpcdet": {
        "output_name": "second",
        "config": Path(ROOT_DIR)
        / "third_party/OpenPCDet/tools/cfgs/nuscenes_models/cbgs_second_multihead.yaml",
        "checkpoint": Path(ROOT_DIR)
        / "src/detector/second/model/cbgs_second_multihead_nds6229_updated.pth",
    },
}


def default_output_path(detector_name: str, split: str) -> Path:
    output_name = MODEL_DEFAULTS[detector_name]["output_name"]
    return (
        Path(ROOT_DIR)
        / f"src/detector/{output_name}/detections"
        / f"{output_name}_openpcdet_nuscenes_simpletrack_{split}.json"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Export nuScenes detections from an OpenPCDet detector in the "
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "Destination SimpleTrack JSON. Defaults to "
            "src/detector/<model-folder>/detections/"
            "<model-folder>_openpcdet_nuscenes_simpletrack_<split>.json."
        ),
    )
    parser.add_argument(
        "--checkpoint-jsonl",
    ...
    defaults = MODEL_DEFAULTS[args.detector_name]
...
def main() -> None:
    ...
    expected_frame_count = args.max_samples or len(dataset)
    if args.max_samples is not None:
        expected_frame_count = min(args.max_samples, len(dataset))
    if completed_tokens:
        dataset.infos = [
            info for info in dataset.infos if info["token"] not in completed_tokens
        ]
