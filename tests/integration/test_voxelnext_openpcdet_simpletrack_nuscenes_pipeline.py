...
class TestVoxelNextOpenpcdetSimpleTrack(unittest.TestCase):
    def test_predict_and_evaluate_from_inference_engine_with_nuscenes_mini(self):
        checkpoint_file = f"{ROOT_DIR}/src/detector/voxelnext/model/voxelnext_nuscenes_kernel1.pth"
        ...
        if not any(key.startswith(("vfe.", 
            ...
            raise unittest.SkipTest(f"OpenPCDet-compatible VoxelNeXt checkpoint is missing: "
                                     f"{checkpoint_file}")
        
        settings = generate_nuscenes_mini_settings_with_custom_detector_and_custom_tracker(detector_name="voxelnext",
        ...
        predictions = inference_engine.predict(
            detector_name="voxelnext_openpcdet",
